#!/usr/bin/env python3
"""
Extract and generate YAML models for all STM32L5 family members.
...
# Define STM32L5 family information
STM32L5_FAMILIES = {
    'L55x': {
        'chips': ['STM32L552'],
        'cores': {'': None},
...
FUNCTIONAL_BLOCKS = frozenset({
    'ADC', 'ADC_Common', 'AES', 'COMP', 'CRC', 'CRS', 'DAC',
    'DBGMCU', 'DFSDM', 'DMA', 'DMAMUX', 'EXTI', 'FDCAN',
    'Flash', 'FMC', 'GPIO', 'HASH', 'I2C', 'IWDG',
    'LPTIM', 'LPUART', 'OCTOSPI', 'OPAMP', 'OTFDEC', 'PKA',
    'PWR', 'RCC', 'RNG', 'RTC', 'SAI', 'SDMMC', 'SPI', 'SYSCFG',
...
def get_canonical_name(periph_name, periph_obj=None):
    ...
    if periph_name.startswith('GTZC'):
        return None
    # Skip cache controllers and debug control block
    if periph_name in ('ICache', 'DCB'):
        return None
    ...
    if periph_name == 'FLASH':
        return 'Flash'
    if periph_name == 'DMAMUX1':
        return 'DMAMUX'
    if periph_name == 'FDCAN1':
        return 'FDCAN'
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32L5_svd_V1.0/{svd_filename}.svd'
...
def main():
    if len(sys.argv) < 3:
        print("Usage: generate_stm32l5_models.py <zip_path> <output_dir>")
        sys.exit(1)
    ...
    print(f"Extracting STM32L5 models from {zip_path}")
    ...
    for family_name, family_info in STM32L5_FAMILIES.items():
    ...
    common_blocks_dir = output_dir / 'L5'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32L5_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> L5 (shared across all families)")
                continue

        for family_name in STM32L5_FAMILIES:
            if family_name in families_present:
                family_specific_count += 1
                family_dir = output_dir / 'L5' / family_name
    ...
    print(f"  Common blocks (L5):     {common_count}")
