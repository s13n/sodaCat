#!/usr/bin/env python3
"""
Extract and generate YAML models for all STM32F3 family members.
...
# Define STM32F3 family information
STM32F3_FAMILIES = {
    'F30x': {
        'chips': ['STM32F301', 'STM32F302', 'STM32F303'],
        'cores': {'': None},
    },
    'F37x': {
        'chips': ['STM32F373'],
...
FUNCTIONAL_BLOCKS = frozenset({
    'ADC', 'ADC_Common', 'bxCAN', 'CEC', 'CRC', 'DAC', 'DBGMCU',
    'DMA', 'EXTI', 'Flash', 'FMC', 'GPIO',
    'HRTIM_Common', 'HRTIM_Master', 'HRTIM_Timer',
    'I2C', 'IWDG', 'OPAMP', 'PWR', 'RCC', 'RTC', 'SDADC',
    'SPI', 'SYSCFG', 'AdvCtrlTimer', 'GpTimer', 'BasicTimer',
    'TSC', 'USART', 'USB', 'WWDG',
})
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32F3_SVD/STM32F3_svd_V1.3/{svd_filename}.svd'
...
def main():
    if len(sys.argv) < 3:
        print("Usage: generate_stm32f3_models.py <zip_path> <output_dir>")
        sys.exit(1)
    ...
    print(f"Extracting STM32F3 models from {zip_path}")
    ...
    for family_name, family_info in STM32F3_FAMILIES.items():
    ...
    common_blocks_dir = output_dir / 'F3'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32F3_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> F3 (shared across all families)")
                continue

        for family_name in STM32F3_FAMILIES:
            if family_name in families_present:
                family_specific_count += 1
                family_dir = output_dir / 'F3' / family_name
    ...
    print(f"  Common blocks (F3):     {common_count}")
