#!/usr/bin/env python3
"""
Extract and generate YAML models for all STM32G0 family members.
...
# Define STM32G0 family information
STM32G0_FAMILIES = {
    'G03x_G04x': {
        'chips': ['STM32G030', 'STM32G031', 'STM32G041'],
        'cores': {'': None},
    },
    'G05x_G06x': {
        'chips': ['STM32G050', 'STM32G051', 'STM32G061'],
        'cores': {'': None},
    },
    'G07x_G08x': {
        'chips': ['STM32G070', 'STM32G071', 'STM32G081'],
        'cores': {'': None},
    },
    'G0Bx_G0Cx': {
        'chips': ['STM32G0B0', 'STM32G0B1', 'STM32G0C1'],
        'cores': {'': None},
    },
}
...
FUNCTIONAL_BLOCKS = frozenset({
    'ADC', 'AES', 'CEC', 'COMP', 'CRC', 'DAC', 'DBGMCU',
    'DMA', 'DMAMUX', 'EXTI', 'FDCAN', 'Flash', 'GPIO',
    'I2C', 'IWDG', 'LPTIM', 'LPUART', 'PWR', 'RCC', 'RNG', 'RTC',
    'SPI', 'SYSCFG', 'SYSCFG_ITLINE', 'TAMP',
...
def get_canonical_name(periph_name, periph_obj=None):
    ...
    if periph_name == 'FLASH':
        return 'Flash'
    if periph_name == 'HDMI_CEC':
        return 'CEC'
    if periph_name == 'SYSCFG_ITLINE':
        return 'SYSCFG_ITLINE'
    ...
    if periph_name.startswith('USART'):
        return 'USART'
    # LPUART (SVD uses LPUART, LPUART1, LPUART2)
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32G0_svd/STM32G0_svd/STM32G0_svd_V1.6/{svd_filename}.svd'
...
def main():
    ...
    print(f"Extracting STM32G0 models from {zip_path}")
    ...
    for family_name, family_info in STM32G0_FAMILIES.items():
    ...
    common_blocks_dir = output_dir / 'G0'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32G0_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> G0 (shared across all families)")
                continue

        for family_name in STM32G0_FAMILIES:
            if family_name in families_present:
                family_specific_count += 1
                family_dir = output_dir / 'G0' / family_name
    ...
    print(f"  Common blocks (G0):     {common_count}")
