#!/usr/bin/env python3
"""
Extract and generate YAML models for all STM32L1 family members.
...
# Define STM32L1 family information
STM32L1_FAMILIES = {
    'L10x': {
        'chips': ['STM32L100'],
        'cores': {'': None},
    ...
    'L15x_L16x': {
        'chips': ['STM32L151', 'STM32L152', 'STM32L162'],
        'cores': {'': None},
    },
}
...
FUNCTIONAL_BLOCKS = frozenset({
    'ADC', 'AES', 'COMP', 'CRC', 'DAC', 'DBGMCU', 'DMA',
    'EXTI', 'Flash', 'FSMC', 'GPIO', 'I2C', 'IWDG', 'LCD',
    'OPAMP', 'PWR', 'RCC', 'RI', 'RTC', 'SDIO', 'SPI', 'SYSCFG',
    'GpTimer', 'BasicTimer',
    'USART', 'USB', 'WWDG',
})
...
def main():
    if len(sys.argv) < 3:
        print("Usage: generate_stm32l1_models.py <zip_path> <output_dir>")
        sys.exit(1)
    ...
    print(f"Extracting STM32L1 models from {zip_path}")
    ...
    for family_name, family_info in STM32L1_FAMILIES.items():
    ...
    common_blocks_dir = output_dir / 'L1'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32L1_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> L1 (shared across all families)")
                continue

        for family_name in STM32L1_FAMILIES:
            if family_name in families_present:
                family_specific_count += 1
                family_dir = output_dir / 'L1' / family_name
    ...
    print(f"  Common blocks (L1):     {common_count}")
