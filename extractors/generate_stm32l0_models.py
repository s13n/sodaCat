#!/usr/bin/env python3
"""
Extract and generate YAML models for all STM32L0 family members.
...
# Define STM32L0 family information
STM32L0_FAMILIES = {
    'L0x0_L0x1': {
        'chips': ['STM32L0x0', 'STM32L0x1'],
        'cores': {'': None},
    },
    'L0x2_L0x3': {
        'chips': ['STM32L052', 'STM32L053', 'STM32L062', 'STM32L063', 'STM32L0x2', 'STM32L0x3'],
        'cores': {'': None},
    },
}
...
FUNCTIONAL_BLOCKS = frozenset({
    'ADC', 'AES', 'CRC', 'CRS', 'DAC', 'DBGMCU', 'DMA', 'EXTI',
    'Flash', 'GPIO', 'I2C', 'IWDG', 'LCD', 'LPTIM', 'LPUART',
    'PWR', 'RCC', 'RNG', 'RTC', 'SPI', 'SYSCFG',
    'GpTimer', 'BasicTimer',
    'TSC', 'USART', 'USB', 'WWDG',
})
...
def get_canonical_name(periph_name, periph_obj=None):
    ...
    # DBG/DBGMCU (SVD uses both across chips)
    if periph_name in ('DBG', 'DBGMCU'):
        return 'DBGMCU'

    # LPUART (SVD uses LPUART1 on some chips, LPUSART1 on others)
    if periph_name in ('LPUART1', 'LPUSART1'):
        return 'LPUART'

    # SYSCFG (SVD uses SYSCFG or SYSCFG_COMP)
    if periph_name in ('SYSCFG', 'SYSCFG_COMP'):
        return 'SYSCFG'
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32L0_svd/STM32L0_svd_v1r4/{svd_filename}.svd'
...
def main():
    if len(sys.argv) < 3:
        print("Usage: generate_stm32l0_models.py <zip_path> <output_dir>")
        sys.exit(1)
    ...
    print(f"Extracting STM32L0 models from {zip_path}")
    ...
    for family_name, family_info in STM32L0_FAMILIES.items():
    ...
    common_blocks_dir = output_dir / 'L0'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32L0_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> L0 (shared across all families)")
                continue

        for family_name in STM32L0_FAMILIES:
            if family_name in families_present:
                family_specific_count += 1
                family_dir = output_dir / 'L0' / family_name
    ...
    print(f"  Common blocks (L0):     {common_count}")
