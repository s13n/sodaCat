#!/usr/bin/env python3
"""
Generic extractor: generate YAML models for any NXP LPC family.

Usage: generate_lpc_models.py <family_code> <svd_dir> <output_dir> [--audit]

The family_code selects a family from svd/NXP/LPC.yaml, which provides the
subfamily-to-chip mapping and block definitions (source instances, interrupt
mappings, transforms, parameters) for that family.

SVD files are read from the NXP SVD repository checkout at <svd_dir>.
Each chip has a folder <chip>/<chip>.xml within the repo.
"""
...
def _resolve_chip_param(chip_params, subfamily, chip, instance, block_type, param_name, default=None):
    """Resolve a parameter value using the 7-level cascade."""
...
def _build_config_interrupt_mapping(blocks_config, shared_blocks):
    ...
    NXP interrupt names (e.g. PIN_INT0 for PINT) don't always follow the
    algorithmic prefix-stripping patterns. This mapping provides a direct
    config-driven lookup that is tried before the algorithmic resolution.
    ...
    # Also include shared block mappings
    for bt, bc in shared_blocks.items():
...
def _resolve_interrupt_name(raw_name, instance_name, canonical_names):
    ...
    # 5. Instance-name-as-interrupt: single canonical name
...
def _find_svd_file(svd_dir, chip_name):
    """Locate an SVD file in the NXP repo structure.
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate YAML models for an NXP LPC family from SVD files.')
    parser.add_argument('family_code', help='Family code (e.g. LPC8, LPC54)')
    parser.add_argument('svd_dir', type=Path, help='Path to NXP SVD repository checkout')
    ...
    if not svd_dir.exists():
        print(f"Error: {svd_dir} not found")
        sys.exit(1)
    ...
    print(f"Extracting {family_code} models from {svd_dir}")
    ...
    for family_name, family_info in families.items():
        ...
        # Resolve block configs for this subfamily
        effective_blocks = {}
        for bt, bc in blocks_config.items():
    ...
    chip_model_count = 0
    unmatched_interrupts = defaultdict(set)
    ...
    for subfamily_name, subfamily_info in families.items():
        ...
        # Determine model file names per block type
        block_model_names = {}
        for bt, bc in blocks_config.items():
            resolved = _resolve_block_config(bc, subfamily_name)
        ...
        for chip_name in subfamily_info['chips']:
            ...
            for inst_name, periph in summary['peripherals'].items():
                ...
                # Map interrupts: try config-driven mapping first, then algorithmic
                mapped_intrs = []
                for raw_intr in periph.get('interrupts', []):
                    raw_name = raw_intr['name']
                    # Config-driven direct lookup
    ...
    if args.audit:
        ...
        if noops or partials:
            ...
            print(f"Review these in svd/NXP/LPC.yaml and svd/NXP/SVD_ERRATA.md")
        else:
