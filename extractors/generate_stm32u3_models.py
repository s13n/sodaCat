...
def get_canonical_name(periph_name, periph_obj=None):
    ...
    if periph_name == 'ICACHE':
        return None
    # Skip USB SRAM (memory-mapped buffer, not a register peripheral)
    ...
    # Normalize ExTI -> EXTI (U3 SVD uses unusual casing)
    if periph_name == 'ExTI':
        return 'EXTI'
    ...
    # I3C (check before I2C since I3C doesn't start with I2C)
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32U3_svd_V1r1/{svd_filename}.svd'
...
def main():
    ...
    common_blocks_dir = output_dir / 'U3_common'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> U3_common (shared across all families)")
                continue
    ...
    print(f"  Common blocks (U3_common):     {common_count}")
