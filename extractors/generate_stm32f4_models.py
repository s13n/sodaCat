...
def get_canonical_name(periph_name, periph_obj=None):
    ...
    # CAN bus
    if periph_name in ('CAN1', 'CAN2', 'CAN3'):
        return 'bxCAN'
    ...
    # FMPI2C (keep as its own block type, check before I2C)
    if periph_name.startswith('FMPI2C'):
        return 'FMPI2C'

    # LPTIM
    if periph_name in ('LPTIM', 'LPTIM1') or periph_name.startswith('LPTIM'):
        return 'LPTIM'
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32F4_svd/STM32F4_svd_V2.0/{svd_filename}.svd'
...
def main():
    ...
    common_blocks_dir = output_dir / 'F4_common'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32F4_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> F4_common (shared across all families)")
                continue
    ...
    print(f"  Common blocks (F4_common):     {common_count}")
