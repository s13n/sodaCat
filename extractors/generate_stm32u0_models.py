...
FUNCTIONAL_BLOCKS = frozenset({
    'ADC', 'AES', 'COMP', 'CRC', 'CRS', 'DAC', 'DBGMCU',
    'DMA', 'DMAMUX', 'EXTI', 'Flash', 'GPIO', 'I2C', 'IWDG',
    'LCD', 'LPTIM', 'LPUART', 'OPAMP', 'PWR', 'RCC', 'RNG', 'RTC',
    'SPI', 'SYSCFG', 'TAMP', 'AdvCtrlTimer', 'GpTimer', 'BasicTimer',
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32U0_svd/STM32U0_svd_V1.0/{svd_filename}.svd'
...
def main():
    ...
    common_blocks_dir = output_dir / 'U0_common'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32U0_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> U0_common (shared across all families)")
                continue
    ...
    print(f"  Common blocks (U0_common):     {common_count}")
