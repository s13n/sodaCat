...
Three-pass processing: Pass 1 collects blocks from all chips (resolving per-subfamily config via `variants`); Pass 2 writes block models using config-driven placement, injecting canonical output names from the family or shared-block `interrupts:` mapping into each model's `outputs:` list so it declares the superset of all consuming families' interrupt outputs; Pass 3 generates chip models with per-instance `outputs:` (canonical signal → `NVIC.<abs_vec>` destinations), `instances:`, `parameters:`, and `models:`.  The `interrupts:` key in the family config remains the SVD-raw → canonical name map — it stays interrupt-specific because SVD's `<interrupt>` elements only describe NVIC outputs; non-interrupt outputs (DMA, wakeups) are not in SVD and will gain their own config keys in Phase 2.
