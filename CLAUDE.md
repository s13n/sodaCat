...
The `validate_chip_interrupts.py` validator strict-checks that every per-instance connection signal name appears in the referenced block's `outputs:` list, and that every destination parses as `<instance>.<port>` — with an integer `port` when the instance is `NVIC` (other targets' port grammars get their own checks alongside their schema definitions in Phase 2).
