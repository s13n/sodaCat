"""Raspberry Pi vendor extension for the unified model generator.

Provides SVD access via loose files in a directory.  RP2040 and RP2350 SVD
files use the chip name directly as the filename (e.g. RP2040.svd).
Requires tools/ on sys.path (set up by generate_models.py).
"""

import sys
from pathlib import Path

import svd

# Use config-driven interrupt mapping (raw SVD name -> canonical).
use_config_interrupt_map = True


def add_cli_args(parser):
    """Add Raspberry-specific CLI arguments."""
    parser.add_argument('svd_source', type=Path,
                        help='Path to directory containing Raspberry Pi SVD files')


def validate_args(args):
    """Validate Raspberry-specific arguments."""
    if not args.svd_source.exists():
        print(f"Error: {args.svd_source} not found")
        sys.exit(1)


def config_path(args):
    """Return path to the Raspberry consolidated YAML config."""
    return Path(__file__).parent.parent.parent / 'svd' / 'Raspberry' / 'Raspberry.yaml'


def open_svd(args, chip_name):
    """Locate and parse an SVD file from the Raspberry directory.

    Naming convention: chip_name as-is (e.g. RP2040 -> RP2040.svd).

    Returns (xml_root, extra_metadata_dict) or None if not found.
    """
    svd_dir = Path(args.svd_source)
    svd_path = svd_dir / f"{chip_name}.svd"

    if not svd_path.exists():
        # Fallback: try lowercase
        svd_path = svd_dir / f"{chip_name.lower()}.svd"
        if not svd_path.exists():
            return None

    root = svd.parse(str(svd_path))
    return root, {}


def format_source(name, version, svd_tag=''):
    """Format source attribution string."""
    if version:
        return f"{name} SVD v{version}"
    return f"{name} SVD"


def errata_path():
    """Return the vendor's SVD errata doc path (for audit reports)."""
    return 'svd/Raspberry/SVD_ERRATA.md'


def get_interrupt_offset(cpu_info):
    """Return interrupt vector offset for Cortex-M (16 system exceptions)."""
    return 16
