...
Phase 2 (planned) will introduce DMAMUX / EXTI / trigger-crossbar destinations.  The schema and consumer code already accept them — Phase 1 deliberately defers the C++ field-type dispatch (today every output emits as `Exception ex<NAME>`) until there's actual heterogeneity to dispatch on.

The `validate_chip_interrupts.py` validator strict-checks that every per-instance output name appears in the referenced block's `outputs:` list, and that every destination parses as `<instance>.<port>` — with an integer `port` when the instance is `NVIC` (other targets' port grammars get their own checks alongside their schema definitions in Phase 2).
...
- The peripheral-instance map (`instances:`) and the block-model index (`models:`) follow an intersection pattern: entries identical across every chip in the subfamily lift to the subfamily YAML; chip YAMLs keep only their deltas.  Per-instance `outputs:` (interrupt vectors, DMA request lines, ...) is part of the instance entry, so the vector-table merging is implicit in the instance lift — an instance whose entire entry (baseAddress + outputs + parameters) is byte-identical across every chip lifts wholesale.  For multi-chip subfamilies the chip YAMLs typically retain just a handful of entries (e.g. H745_H757 chip YAMLs each carry 6 of 114 instances).  Patches still apply per-chip first (block-level `outputs:` mappings, `chip_outputs:` overrides, `chip_instances:` exclusions) — the intersection sees the post-patch state.  Chip-level entries override ancestor entries by name; the chip-header generator walks the `inherits:` chain to assemble the merged view.
...
- `chip_outputs` (optional): subfamily-keyed output-routing overrides to inject or correct chip-level signal destinations. Same cascade structure as `chip_params`: `chip_outputs[subfamily|_all][chip|_all][instance_name] = {canonical_name: [destination_string, ...]}`. Each destination is a dotted `instance.port` string (e.g. `NVIC.53`, `DMAMUX1.42`). First match wins (no merging across levels). An override entry REPLACES the destination list for that signal — it does not merge with SVD-derived NVIC destinations. Used to fix SVD bugs (misattributed/missing/misnumbered interrupts) and to wire non-SVD outputs (DMA requests, wakeups) the SVD doesn't describe.
- `chip_instances` (optional): per-chip instance presence overrides, structured as `chip_instances[subfamily|_all][chip] = {exclude: [instance_names...]}`. Drops named instances from the chip model, including their entries in `instances:` (along with their per-instance `outputs:`) and (transitively) `models:`. Used when a single SVD covers a family superset but individual chips lack peripherals (e.g. LPC4310 has no USB; LPC4320 has USB0 but not USB1). Family-wide or subfamily-wide gaps belong in the per-block `instances:` list instead — `chip_instances` is for chip-specific deviations only.
