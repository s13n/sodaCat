#!/usr/bin/env python3
"""
Extract and generate YAML models for all STM32C0 family members.
...
STM32C0_FAMILIES = {
    'C01x_C03x': {
        'chips': ['STM32C011', 'STM32C031'],
        'cores': {'': None},
    },
    'C05x_C07x': {
        'chips': ['STM32C051', 'STM32C071'],
        'cores': {'': None},
    },
    'C09x': {
        'chips': ['STM32C091', 'STM32C092'],
        'cores': {'': None},
    },
}
...
def extract_svd_from_zip(zip_path, svd_filename):
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zf:
            svd_path = f'STM32C0_svd_V1.5/{svd_filename}.svd'
...
def main():
    if len(sys.argv) < 3:
        print("Usage: generate_stm32c0_models.py <zip_path> <output_dir>")
        sys.exit(1)
    ...
    print(f"Extracting STM32C0 models from {zip_path}")
    ...
    for family_name, family_info in STM32C0_FAMILIES.items():
    ...
    common_blocks_dir = output_dir / 'C0'
    ...
    for block_name in sorted(all_blocks.keys()):
        ...
        all_families = set(STM32C0_FAMILIES.keys())
        ...
        if families_present == all_families:
            ...
            if all_hashes_same:
                ...
                print(f"  + {block_name:20} -> C0 (shared across all families)")
                continue

        for family_name in STM32C0_FAMILIES:
            if family_name in families_present:
                family_specific_count += 1
                family_dir = output_dir / 'C0' / family_name
    ...
    print(f"  Common blocks (C0):     {common_count}")
