...
apb3enr = svd.findNamedEntry(rcc['registers'], 'APB3ENR')
apb3enr['fields'].append({ 'name': 'DSIEN', 'bitOffset': 4, 'description': 'DSI peripheral clocks enable'})
transform.renameEntries(rcc['registers'], 'name', r'^(RSR)', r'C0_\1')
transform.renameEntries(rcc['registers'], 'name', r'^(A[HP]B.+ENR)', r'C0_\1')
rcc['registers'] = transform.createClusterArray(rcc['registers'], r"C(\d+)_(.+?)$", {'name': 'C', 'description': 'CPU-specific registers; 0 = this CPU, 1 = CM7, 2 = CM4'})
rcc_c = svd.findNamedEntry(rcc['registers'], 'C[%s]')
rcc_c['dim'] = 3
