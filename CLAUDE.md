...
Three-pass processing: Pass 1 collects blocks from all chips (resolving per-subfamily config via `variants`); Pass 2 writes block models using config-driven placement, injecting canonical output names from the family or shared-block `outputs:` mapping into each model's `outputs:` list so it declares the superset of all consuming families' outputs; Pass 3 generates chip models with per-instance `connections:` (canonical signal → `[destination_string, ...]` lists), `instances:`, `parameters:`, and `models:`.  The block-level `outputs:` key plays two roles in the same map: SVD-raw → canonical name for interrupt outputs (driven by `<interrupt>` elements), and direct declaration of canonical names for non-SVD outputs (DMA requests, wakeups) where the raw key typically equals the canonical name and only the dict-value form (`{name, description}`) carries description text.  Per-instance routing of those signals is set in `chip_connections:` rather than in this block-level declaration map.  The block declares what comes OUT of the block; the chip says what those outputs CONNECT TO.
