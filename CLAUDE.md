...
- The peripheral-instance map (`instances:`) and the block-model index (`models:`) follow the same intersection pattern: entries identical across every chip in the subfamily lift to the subfamily YAML; chip YAMLs keep only their deltas.  For multi-chip subfamilies the chip YAMLs typically retain just a handful of entries (e.g. H745_H757 chip YAMLs each carry 6 of 114 instances).  Chip-level entries override ancestor entries by name; the chip-header generator walks the `inherits:` chain to assemble the merged view.
